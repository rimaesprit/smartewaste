...
class FrontController extends AbstractController
{
    ...
    {
        // Log pour débogage
        error_log('Accès à la page d\'accueil front');
        
        // Vérifier si l'utilisateur est connecté pour débogage
        $user = $this->getUser();
        if ($user) {
            error_log('Utilisateur connecté: ' . $user->getUserIdentifier());
        } else {
    ...
    public function submitAvis(Request $request, ?Poubelle $poubelle, EntityManagerInterface $entityManager): Response
    {
        $bienetre = new Bienetre();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                ...
                return $this->redirectToRoute('app_front_bienetre_by_poubelle', [
                    'id' => $bienetre->getPoubelle()->getId()
                ]);
            } catch (\Exception $e) {
                $this->addFlash('error', 'Une erreur est survenue lors de l\'enregistrement de votre avis. Veuillez réessayer.');
            }
        }

        return $this->render('bienetre/front_new.html.twig', [
    ...
    #[Route('/parking-info', name: 'app_front_parking', methods: ['GET'])]
    public function parkingInfo(ParkingRepository $parkingRepository): Response
    {
    ...
    #[Route('/parking-heatmap', name: 'app_front_parking_heatmap')]
    public function parkingHeatmap(): Response
    {
        return $this->render('front/parking-heatmap.html.twig');
    }
}